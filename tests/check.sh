...
# failed is a decoration: each linter and gate against a known-bad input, the behaviour
# assertions through a probe their own helpers have to reject
set -euo pipefail
...
Nothing here reaches the network: contrib.sh is driven through tests/fixtures/fake-gh,
which answers from files
...
cmd_lint() {
  ...
  # repositories that declare the same name replace each other, together with whatever was
  # installed from the one that lost. One repository holds the list, and it holds the check
  # that each entry matches the skill it names
  [ ! -e .claude-plugin ] ||
