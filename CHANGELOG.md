...
- `contrib.sh`, over `gh` and `jq`: `repo` for a project's contribution policy in one lookup, `dupes` for the duplicate search, `draft`, `drafts` and `send` for the gate, `status` and `seen` for what changed on the user's own pull requests and issues, and `home` for where the private files live
