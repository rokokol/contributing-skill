...
- `contrib.sh` and `tests/check.sh`'s `--help` now carries what a caller needs, the network line included — `Reaches GitHub only through gh`, `Nothing here reaches the network: contrib.sh is driven through tests/fixtures/fake-gh, which answers from files` — which the entry above said stayed in the header; the header keeps only what an editor needs, the bash and tool line among them
